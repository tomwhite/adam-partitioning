...
public class CrunchPartitionAvroTool extends Configured implements Tool {
  ...
  @Override
   public int run(String[] args) throws Exception {
    ...
    Pipeline pipeline = new MRPipeline(CrunchPartitionAvroTool.class, conf);
    PCollection<GenericData.Record> records = pipeline.read(From.avroFile(inputPath));
  ...
  public static void main(String[] args) throws Exception {
    int exitCode = ToolRunner.run(new CrunchPartitionAvroTool(), args);
  ...
  private PartitionStrategy readPartitionStrategy(String name) throws IOException {
    InputStream in = CrunchPartitionAvroTool.class.getResourceAsStream(name + ".json");
    try {
